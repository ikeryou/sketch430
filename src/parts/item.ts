...
export class Item extends MyDisplay {
  ...
  protected _update():void {
    ...
    const fontSize = Func.val(40, 20) * Util.map(this._dist, 0.75, 1.5, 0, maxDist)
    ...
    const it = Util.map(this._dist, 0, 4, 0, maxDist)

    const dx = MousePointer.instance.old.x - MousePointer.instance.x
    const dy = MousePointer.instance.old.y - MousePointer.instance.y

    // 向き
    const tgRot = Util.radian(Util.degree(Math.atan2(dy, dx * -1)) - 90);
    const PI2 = Math.PI * 2;
    const PI3 = Math.PI * 3;
    const diff = (tgRot - this._rot.z % PI2 + PI3) % PI2 - Math.PI
    const newtgRot = this._rot.z + diff
    this._rot.z += (newtgRot - this._rot.z) * ease
    const ang = Util.degree(this._rot.z) + 0

    const hsl = new HSL()
    hsl.s = 1
    hsl.l = 0.5
    hsl.h = Util.map(this._dist, 0, 3, 0, maxDist)
    const col = new Color().setHSL(hsl.h, hsl.s, hsl.l)

    const txtCol = col.clone()
    txtCol.offsetHSL(0.5, 0, 1)
    ...
    Tween.set(this.el, {
      ...
      rotationZ: Math.sin(this._key * 0.15 + this._c * 0.1) * Util.map(this._dist, 0, 90, 0, maxDist),
